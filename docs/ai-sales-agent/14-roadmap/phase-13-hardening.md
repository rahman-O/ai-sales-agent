...
- [ ] P13-T001: Execute threat-model and tenant-boundary suite. Verification: Every new table/API/job/storage path is covered with two-tenant probes.
- [ ] P13-T002: Run approved baseline/burst/noisy-tenant load test. Verification: Measured latency/queue/fairness meets targets or causes explicit scope revision.
- [ ] P13-T003: Inject worker/Redis/DB/provider failures. Verification: Durable work recovers without duplicate business mutations.
- [ ] P13-T004: Perform isolated backup/restore plus deletion replay. Verification: Measured RPO/RTO and post-restore outbound quarantine are documented.
- [ ] P13-T005: Validate alerts, secret rotation and incident ownership. Verification: On-call can follow alerts to a tested mitigation without hidden knowledge.
- [ ] P13-T006: Close critical findings and record residual risk review. Verification: Release reviewers see reproducible reports and no unowned critical defects.
...
- [ ] Approved load/SLO targets have measured evidence.
- [ ] No critical tenant, booking or false-success defect remains open.
- [ ] Restore drill meets approved recovery goals or blocks pilot.
- [ ] Alerts and kill switches have been exercised by designated operators.
